/*
 * driver for the usb_to_i2c adapter - 1.0
 *
			      ...
			      const struct usb_device_id *id)
{
	...
	snprintf(dev->adapter.name, sizeof(dev->adapter.name),
		 "usb_to_i2c at bus %03d device %03d",
		 dev->usb_dev->bus->busnum, dev->usb_dev->devnum);
...
MODULE_AUTHOR("Sasanka Sekhar Dash <h20170176@pilani.bits-pilani.ac.in>");
MODULE_DESCRIPTION("usb_to_i2c driver v1.0");
MODULE_LICENSE("GPL");
